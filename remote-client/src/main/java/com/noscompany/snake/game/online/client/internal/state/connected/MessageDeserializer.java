    ...
    private Try<DeserializedMessage> mapToObject(String serializedMessage, MessageType messageType) {
        return switch (messageType) {
            case GAME_REINITIALIZED -> Try
                    ...
                    .map(event -> new DeserializedMessage(messagePublisher -> messagePublisher.publishMessage(event)));
            case INITIALIZE_PLAYGROUND_STATE -> Try
                    .of(() -> objectMapper.readValue(serializedMessage, InitializePlaygroundToRemoteClient.class))
                    .map(event -> new DeserializedMessage(messagePublisher -> messagePublisher.publishMessage(event)));
