...
class MessageMapperImpl implements MessageMapper {
    ...
    private Message toTakeASeat(String message) {
        return tryToMessage(
                () -> {
                    String snakeNumberStr = JsonPath.parse(message).read("$.snakeNumber");
                    return new TakeASeat(SnakeNumber.valueOf(snakeNumberStr));
                },
                message);
    }
...
class EnterRoom implements Message {
    ...
    @Override
    public void onTakeASeat(Consumer<SnakeNumber> consumer) {
